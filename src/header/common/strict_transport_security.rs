...
impl FromStr for StrictTransportSecurity {
    ...
    fn from_str(s: &str) -> crate::Result<StrictTransportSecurity> {
        s.split(';')
            .map(str::trim)
            ...
            .fold(Ok((None, None)), |res, dir| match (res, dir) {
                ...
                (Ok((Some(_), _)), Ok(Directive::MaxAge(_))) => Err(crate::Error::Header),
                (Ok((_, Some(_))), Ok(Directive::IncludeSubdomains)) => Err(crate::Error::Header),
                (_, Err(_)) => Err(crate::Error::Header),
                (res, _) => res
            })
...
impl Header for StrictTransportSecurity {
    ...
    fn parse_header(raw: &[Vec<u8>]) -> crate::Result<StrictTransportSecurity> {
