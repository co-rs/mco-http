...
use crate::header::{self, Header, HeaderFormat, EntityTag, HttpDate};
...
impl Header for IfRange {
    ...
    fn parse_header(raw: &[Vec<u8>]) -> crate::Result<IfRange> {
        let etag: crate::Result<EntityTag> = header::parsing::from_one_raw_str(raw);
        if etag.is_ok() {
            return Ok(IfRange::EntityTag(etag.unwrap()));
        }
        let date: crate::Result<HttpDate> = header::parsing::from_one_raw_str(raw);
        if date.is_ok() {
