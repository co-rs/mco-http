...
use crate::header::parsing::{from_one_raw_str, from_comma_delimited};
...
impl FromStr for ByteRangeSpec {
    ...
    fn from_str(s: &str) -> crate::Result<ByteRangeSpec> {
        let mut parts = s.splitn(2, "-");

        match (parts.next(), parts.next()) {
            (Some(""), Some(end)) => {
                end.parse().or(Err(crate::Error::Header)).map(ByteRangeSpec::Last)
            },
            (Some(start), Some("")) => {
                start.parse().or(Err(crate::Error::Header)).map(ByteRangeSpec::AllFrom)
            },
            (Some(start), Some(end)) => {
...
impl Header for Range {
    ...
    fn parse_header(raw: &[Vec<u8>]) -> crate::Result<Range> {
