...
/// # Examples
/// ```
/// use cogo_http::header::{Headers, Link, LinkValue, RelationType};
///
...
impl Header for Link {
    ...
    fn parse_header(raw: &[Vec<u8>]) -> crate::Result<Link> {
        ...
        raw.iter()
            ...
            .fold(None, |p, c| match (p, c) {
                ...
                _ => Some(Err(crate::Error::Header)),
            })
            .unwrap_or(Err(crate::Error::Header))
    }
}
...
impl FromStr for Link {
    ...
    fn from_str(s: &str) -> crate::Result<Link> {
            ...
            if segment.trim().starts_with('<') {
                link_values.push(
                    match verify_and_trim(segment.trim(), (b'<', b'>')) {
                        Err(_) => return Err(crate::Error::Header),
                        Ok(s) => {
                            LinkValue {
                ...
                if "rel".eq_ignore_ascii_case(link_param_name) {
                    ...
                    if link_header.rel.is_none() {
                        link_header.rel = match link_param_split.next() {
                            None => return Err(crate::Error::Header),
                            Some("") =>  return Err(crate::Error::Header),
                            Some(s) => {
                                s.trim_matches(|c: char| c == '"' || c.is_whitespace())
                                    .split(' ')
                                    .map(|t| t.trim().parse())
                                    .collect::<Result<Vec<RelationType>, _>>()
                                    .or_else(|_| return Err(crate::Error::Header))
                                    .ok()
                            },
                        };
                    }
...
impl FromStr for MediaDesc {
    ...
    fn from_str(s: &str) -> crate::Result<MediaDesc> {
        match s {
...
impl FromStr for RelationType {
    ...
    fn from_str(s: &str) -> crate::Result<RelationType> {
...
fn verify_and_trim(s: &str, b: (u8, u8)) -> crate::Result<&str> {
    let length = s.len();
