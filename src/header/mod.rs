...
impl<'a, 'b> MultilineFormatter<'a, 'b> {
    fn fmt_line(&mut self, line: &dyn fmt::Display) -> fmt::Result {
        use std::fmt::Write;
        match self.0 {
...
mod sealed {
    ...
    pub trait Sealed {
        #[doc(hidden)]
        fn clone_box(&self) -> Box<dyn HeaderFormat + Send + Sync>;
    }
...
impl dyn HeaderFormat + Send + Sync {
    #[inline]
...
impl<'a> fmt::Display for &'a (dyn HeaderFormat + Send + Sync) {
    #[inline]
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
mod tests {
    ...
    impl Header for CrazyLength {
        ...
        fn parse_header(raw: &[Vec<u8>]) -> crate::Result<CrazyLength> {
