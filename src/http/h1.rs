...
#[derive(Debug)]
enum Stream {
    Idle(Box<dyn NetworkStream + Send>),
    Writing(HttpWriter<BufWriter<Box<dyn NetworkStream + Send>>>),
    Reading(HttpReader<BufReader<Box<dyn NetworkStream + Send>>>),
}

impl Stream {
    fn writer_mut(&mut self) -> Option<&mut HttpWriter<BufWriter<Box<dyn NetworkStream + Send>>>> {
        match *self {
            Stream::Writing(ref mut writer) => Some(writer),
            _ => None,
        }
    }
    fn reader_mut(&mut self) -> Option<&mut HttpReader<BufReader<Box<dyn NetworkStream + Send>>>> {
        match *self {
            Stream::Reading(ref mut reader) => Some(reader),
            _ => None,
        }
    }
    fn reader_ref(&self) -> Option<&HttpReader<BufReader<Box<dyn NetworkStream + Send>>>> {
        match *self {
    ...
    fn new(stream: Box<dyn NetworkStream + Send>) -> Stream {
        Stream::Idle(stream)
    }
}
...
impl HttpMessage for Http11Message {
    ...
    #[inline]
    fn close_connection(&mut self) -> crate::Result<()> {
        r#try!(self.get_mut().close(Shutdown::Both));
        Ok(())
    }
...
impl Http11Message {
    ...
    pub fn into_inner(self) -> Box<dyn NetworkStream + Send> {
    ...
    /// `Http11Message`.
    pub fn get_ref(&self) -> &(dyn NetworkStream + Send) {
    ...
    /// `Http11Message`.
    pub fn get_mut(&mut self) -> &mut (dyn NetworkStream + Send) {
    ...
    /// the peer.
    pub fn with_stream(stream: Box<dyn NetworkStream + Send>) -> Http11Message {
        Http11Message {
            is_proxied: false,
            method: None,
    ...
    pub fn flush_outgoing(&mut self) -> crate::Result<()> {
        let mut res = Ok(());
        self.stream.map_in_place(|stream| {
...
impl Protocol for Http11Protocol {
    fn new_message(&self, host: &str, port: u16, scheme: &str) -> crate::Result<Box<dyn HttpMessage>> {
        let stream = r#try!(self.connector.connect(host, port, scheme)).into();
...
struct Connector(Box<dyn NetworkConnector<Stream=Box<dyn NetworkStream + Send>> + Send + Sync>);
        ...
        match *self {
            SizedReader(ref mut body, ref mut remaining) => {
                ...
                if *remaining == 0 {
                    ...
                    let num = r#try!(body.read(&mut buf[..to_read])) as u64;
            ...
            ChunkedReader(ref mut body, ref mut opt_remaining) => {
                let mut rem = match *opt_remaining {
                    ...
                    None => r#try!(read_chunk_size(body))
                };
                trace!("Chunked read, remaining={:?}", rem);

                if rem == 0 {
                    if opt_remaining.is_none() {
                        r#try!(eat(body, LINE_ENDING.as_bytes()));
                    }
                ...
                let count = r#try!(body.read(&mut buf[..to_read])) as u64;
    ...
    fn write(&mut self, msg: &[u8]) -> io::Result<usize> {
        match *self {
            ...
            ChunkedWriter(ref mut w) => {
                ...
                r#try!(write!(w, "{:X}{}", chunk_size, LINE_ENDING));
            ...
            SizedWriter(ref mut w, ref mut remaining) => {
                ...
                if len > *remaining {
                    let len = *remaining;
                    r#try!(w.write_all(&msg[..len as usize]));
                    *remaining = 0;
                    Ok(len as usize)
                } else {
                    r#try!(w.write_all(msg));
...
fn parse<R: Read, T: TryParse<Subject=I>, I>(rdr: &mut BufReader<R>) -> crate::Result<Incoming<I>> {
    loop {
        match r#try!(try_parse::<R, T, I>(rdr)) {
        ...
        if n == 0 {
            ...
            if buffered == crate::buffer::MAX_BUFFER_SIZE {
