...
pub trait HttpMessage: Write + Read + Send + Any + Typeable + Debug {
    ...
    /// body of the request.
    fn set_outgoing(&mut self, head: RequestHead) -> crate::Result<RequestHead>;
...
impl dyn HttpMessage {
    ...
    unsafe fn downcast_unchecked<T: 'static>(self: Box<dyn HttpMessage>) -> Box<T>  {
        let raw: *mut dyn HttpMessage = mem::transmute(self);
