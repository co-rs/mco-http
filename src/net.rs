...
pub trait NetworkListener: Clone {
    ...
    fn accept(&mut self) -> crate::Result<Self::Stream>;
...
impl<'a, N: NetworkListener + 'a> Iterator for NetworkConnections<'a, N> {
    type Item = crate::Result<N::Stream>;
    fn next(&mut self) -> Option<crate::Result<N::Stream>> {
        Some(self.0.accept())
    }
}
...
impl fmt::Debug for Box<dyn NetworkStream + Send> {
...
pub trait SslClient<T: NetworkStream + Send + Clone = HttpStream> {
    ...
    fn wrap_client(&self, stream: T, host: &str) -> crate::Result<Self::Stream>;
}
...
pub trait SslServer<T: NetworkStream + Send + Clone = HttpStream> {
    ...
    fn wrap_server(&self, stream: T) -> crate::Result<Self::Stream>;
}
...
impl<S: SslServer + Clone> NetworkListener for HttpsListener<S> {
    ...
    #[inline]
    fn accept(&mut self) -> crate::Result<S::Stream> {
